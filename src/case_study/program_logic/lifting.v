...
Lemma wp_lift_stuck E Φ e :
  to_val e = None →
  (∀ σ ns κs nt, (⚡={next_state σ}=> |==> state_interp σ ns κs nt) ={E,∅}=∗ ⌜stuck e σ⌝)
  ⊢ WP e @ E ?{{ Φ }}.
Proof.
...
Proof.
  ...
  iSplitL =>//. iModIntro. iMod "Hσ". iModIntro.
...
Lemma wp_lift_atomic_step_fupd {s E1 E2 Φ} e1 :
  to_val e1 = None →
  (∀ σ1 ns κ κs nt, (⚡={next_state σ1}=> |==> state_interp σ1 ns (κ ++ κs) nt) ={E1}=∗
    ⌜if s is NotStuck then reducible e1 σ1 else True⌝ ∗
