...
From nextgen Require Export nextgen_independent pick.
From nextgen Require Import utils nextgen_pointwise gmap_view_transformation.
...
Class wsatGIndS (Σ : gFunctors) (Ω : gTransformations Σ) (A : cmra) (pick: pick_transform_rel A) : Set := WsatGInd {
  wsat_ind_inG : wsatGIndpreS Σ Ω A pick;
...
Global Existing Instances wsat_ind_inG.
...
#[global] Instance into_wsats `{pick: pick_transform_rel A} `{!wsatGIndS Σ Ω A pick} : wsatGS Σ  :=
  WsatG Σ (@into_wsatpres A pick Σ Ω wsat_ind_inG) invariant_name enabled_name disabled_name.
...
Global Existing Instances noTransInG_A_inG noTransInG_B_inG noTransInG_inG wsatGpreS_func.
...
Definition frame_cond {Σ Ω A} `{pick: pick_transform_rel A} {wsat: wsatGIndS Σ Ω A pick} Q (c : C) : Prop  := GenIndependent2Ω Ω c Q.
...
Lemma ownI_alloc_open φ P c :
  (∀ E : gset positive, ∃ i, i ∉ E ∧ φ i) →
  frame_cond P c ->
  wsat ==∗ ∃ i, ⌜φ i⌝ ∗ (ownE {[i]} -∗ wsat) ∗ ownI i P ∗ ownC i c ∗ ownD {[i]}.
Proof.
...
Proof.
  ...
  iModIntro. iExists (WsatGInd _ _ _ _ _ γI γE γD γI'),eq_refl.
  rewrite /wsat /ownE -lock; iFrame.
  iExists ∅,∅. rewrite fmap_empty big_opM_empty. by iFrame.
Qed.

Lemma lc_ind_insert_intro `{lcGIndS Σ Ω} `{noTransInG Σ Ω A} (m : nat) (t : A → A) `{!CmraMorphism t} `{!Idemp equiv t} :
...
Lemma lc_ind_insert_two_intro `{lcGIndS Σ Ω} `{noTwoTransInG Σ Ω A D} (m : nat)
  (t : A → A) (f : D -> D) `{!CmraMorphism t} `{!CmraMorphism f} `{!Idemp equiv t} `{!Idemp equiv f} :
...
Lemma ls_ind_insert_intro `{lcGIndS Σ Ω} `{noTransInG Σ Ω A} (t : A → A) `{!CmraMorphism t} `{!Idemp equiv t} (m : nat) :
...
Lemma ls_ind_insert_two_intro `{lcGIndS Σ Ω} `{noTwoTransInG Σ Ω A B}
  (t : A → A) (f : B → B) `{!CmraMorphism t} `{!CmraMorphism f} `{!Idemp equiv t} `{!Idemp equiv f} (m : nat) :
...
Lemma ownE_ind_insert_intro `{!wsatGIndS Σ Ω A pick} `{noTransInG Σ Ω B} (E: coPset) (t : B → B) `{!CmraMorphism t} `{!Idemp equiv t} :
...
Lemma ownE_ind_insert_two_intro `{!wsatGIndS Σ Ω A pick} `{noTwoTransInG Σ Ω B D}
  (E: coPset) (t : B → B) (f : D -> D) `{!CmraMorphism t} `{!CmraMorphism f} `{!Idemp equiv t} `{!Idemp equiv f} :
...
Lemma ownD_ind_insert_intro `{!wsatGIndS Σ Ω A pick} `{noTransInG Σ Ω B} (E: gset positive) (t : B → B) `{!CmraMorphism t} `{!Idemp equiv t} :
...
Lemma ownD_ind_insert_two_intro `{!wsatGIndS Σ Ω A pick} `{noTwoTransInG Σ Ω B D}
  (E: gset positive) (t : B → B) (f : D -> D) `{!CmraMorphism t} `{!CmraMorphism f} `{!Idemp equiv t} `{!Idemp equiv f} :
...
Lemma frag_invariant_insert_intro `{!wsatGIndS Σ Ω A pick} `{i : noTransInG Σ Ω B} (t : B → B)
  (j : positive) (P : iProp Σ) `{!CmraMorphism t} `{!Idemp equiv t} :
  ownI j P
...
Lemma frag_invariant_insert_two_intro `{!wsatGIndS Σ Ω A pick} `{i : noTwoTransInG Σ Ω B D}
  ...
  (t : B → B) (f : D → D) `{!CmraMorphism t} `{!CmraMorphism f} `{!Idemp equiv t} `{!Idemp equiv f} :
  ownI j P
...
Lemma auth_pick_map_insert_intro `{!wsatGIndS Σ Ω A pick} F (t : A → A) `{!CmraMorphism t} `{!Idemp equiv t} :
...
Lemma frag_pick_map_insert_intro `{!wsatGIndS Σ Ω A pick} (i : positive) (c : C) (t : A → A) `{!CmraMorphism t} `{!Idemp equiv t} :
  ownC i c ⊢
...
Proof.
  iIntros (HCR) "Hc". rewrite /ownC.
  iDestruct (transmap_own_insert_two_left
...
Lemma ownN_insert_intro `{!wsatGIndS Σ Ω A pick} (i : positive) (t : A → A) `{!CmraMorphism t} `{!Idemp equiv t} :
...
Proof.
  ...
  iAssert ([∗ map] i↦Q ∈ I, ⚡={transmap_insert_two_inG (inv_pick_transform c) (C_pick c) Ω}=>
             ownN i ∨ (∃ c' : C, ownC i c' ∗ ⌜frame_cond Q c'⌝ ∗ (▷ Q ∗ ownD {[i]} ∨ ownE {[i]})))%I with "[H]" as "H".
